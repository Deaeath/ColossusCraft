        ...
        if (line.startsWith("[") && line.endsWith("]")) {
            ...
            for (String part : parts) {
                ...
                if (itemQuantityPair.length > 1) {
                    try {
                        ...
                        throw new CommandException("Failed to parse count for array element \"" + part + "\".", e);
                    }
                }
            ...
            if (!items.isEmpty()) {
                return new ItemList(items.entrySet().stream()
                        .map(entry -> new ItemTarget(entry.getKey(), entry.getValue()))
                        .toArray(ItemTarget[]::new));
            }
        } else {
            String[] items = line.split(" ");
            if (items.length >= 1 && !items[0].isBlank()) {
                String name = items[0];
                ...
                if (items.length == 2) {
                    try {
                        ...
                        throw new CommandException("Failed to parse the following argument into type " + Integer.class + ": " + items[1] + ".", e);
                    }
