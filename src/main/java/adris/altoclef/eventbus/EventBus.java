...
public class EventBus {
    ...
    private static final List<PendingSubscription> _toAdd = new ArrayList<>();
    ...
    public static <T> void publish(T event) {
        ...
        for (PendingSubscription toAdd : _toAdd) {
            subscribeInternal(toAdd.type, toAdd.subscription);
        }
        _toAdd.clear();
    ...
    public static <T> Subscription<T> subscribe(Class<T> type, Consumer<T> consumeEvent) {
        ...
        if (_lock) {
            _toAdd.add(new PendingSubscription(type, sub));
        } else {
    ...
    private record PendingSubscription(Class type, Subscription subscription) {
    }
}
