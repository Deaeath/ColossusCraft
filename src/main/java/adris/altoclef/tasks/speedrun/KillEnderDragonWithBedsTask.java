            ...
            for (EnderDragon dragon : dragons) {
                ...
                if (perching) {
                    ...
                    if ((!WorldHelper.inRangeXZ(WorldHelper.toVec3d(targetStandPosition), mod.getPlayer().position(), 1)
                            || playerPosition.getY() < targetStandPosition.getY()) && mod.getPlayer().getDeltaMovement().x() == 0 &&
                            mod.getPlayer().getDeltaMovement().y() == 0 && mod.getPlayer().getDeltaMovement().z() == 0) {
                    ...
                    if (!bedPlaced) {
                        ...
                        assert bedfoot != null;
                        Vec3 headPos = dragon.head.getBoundingBox().getCenter(); // dragon.head.position();
                        double dist = headPos.distanceTo(WorldHelper.toVec3d(bedfoot));
                        Debug.logMessage("Dist: " + dist);
                        if (dist < MarvionBeatMinecraftTask.getConfig().dragonHeadCloseEnoughClickBedRange) {
