...
public abstract class Task {
    ...
    public void tick(AltoClef mod, TaskChain parentChain) {
        if (parentChain.getTasks().contains(this)) {
            Debug.logInternal("Task CYCLE DETECTED: " + this);
            return;
        }
        ...
        // We have a sub task
        if (newSub != null) {
            if (newSub.isEqual(this)) {
                Debug.logInternal("Task SELF-SUBTASK DETECTED: " + this);
                return;
            }
            for (Task task : parentChain.getTasks()) {
                if (task != this && task.isEqual(newSub)) {
                    Debug.logInternal("Task SUBTASK CYCLE DETECTED: " + newSub + " equals ancestor " + task);
                    return;
                }
            }
